...
import org.javafxplatform.core.docking.skin.DockingPaneSkin;
...
//    }
    ...
    private void addDockingArea(Iterator<ShortPathPart> path, DockingAreaPane dockingAreaPane, List<DockingAreaPane> removedDockingAreas) {
        ShortPathPart pathPart = path.next();
        adjustLevel(pathPart, removedDockingAreas);
        if (path.hasNext()) {
            DockingSplitPane splitPane = getSplitPane(pathPart);
    ...
    private void addDockingArea(ShortPathPart pathPart, DockingAreaPane dockingArea) {
        ...
        areaPanes.put(pathPart.getPosition(), dockingArea);
    ...
    private DockingSplitPane getSplitPane(ShortPathPart shortPathPart) {
        if (!splitPanes.containsKey(shortPathPart.getPosition())) {
            DockingSplitPane splitPane = new DockingSplitPane(shortPathPart.getPosition(), getActualLevel() + 1,
                    getChildOrientation());
            splitPanes.put(shortPathPart.getPosition(), splitPane);
//            if (areaPanes.containsKey(shortPathPart.getPosition())) {
//                DockingAreaPane areaPane = areaPanes.remove(shortPathPart.getPosition());
//                removeDockingArea(areaPane);
//                splitPane.addDockingArea(shortPathPart, areaPane);
//            }
            add(shortPathPart, splitPane);
        }
...
//        shortPathParts.put(pathPart.getPosition(), pathPart);
        child.setParentSplitPane(this);
        int insertionPoint = Positionables.getInsertionPoint(dockingSplitPaneChildren, child);
...
//    }
    ...
    private void removeDockingArea(Iterator<ShortPathPart> path) {
        ...
        if (path.hasNext()) {
            ...
            DockingAreaPane dockingArea = areaPanes.remove(pathPart.getPosition());
...
//    }
    private void adjustLevel(ShortPathPart pathPart, List<DockingAreaPane> removedDockingAreas) {
...
//        shortPathParts.clear();
    ...
    private void removeAllDockingAreas(List<DockingAreaPane> removedDockingAreas) {
        for (DockingAreaPane dockingArea : areaPanes.values()) {
            removeDockingAreaOnly(dockingArea);
    ...
    private void removeDockingAreaOnly(DockingAreaPane dockingArea) {
        int index = Collections.binarySearch(dockingSplitPaneChildren, dockingArea, new PositionableComparator());
        dockingSplitPaneChildren.remove(index);
        dockingArea.setVisualized(false);
    }
    ...
    private void removeSplitPane(DockingSplitPane splitPane) {
        ...
        int index = Collections.binarySearch(dockingSplitPaneChildren, splitPane, new PositionableComparator());
        dockingSplitPaneChildren.remove(index);
    }
}
