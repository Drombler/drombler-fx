...
public class Main extends org.drombler.acp.startup.main.impl.Main {
     ...
     * switch to set the bundle auto-deploy directory and a single argument to set the bundle cache directory.
     * </li>
     ...
     * before starting the framework. This mechanism is mainly shorthand for people starting the framework from the
     * command line to avoid having to specify a bunch of <tt>-D</tt> system property definitions. The only properties
     * defined in this file that will impact the framework's behavior are the those concerning setting HTTP proxies,
     * such as
     * <tt>http.proxyHost</tt>, <tt>http.proxyPort</tt>, and
     * <tt>http.proxyAuth</tt>. Generally speaking, the framework does not use system properties at all.
     * </li>
     ...
     * system property; this should be set using the <tt>-D</tt> syntax when executing the JVM. If the
     * <tt>config.properties</tt> file cannot be found, then default values are used for all configuration properties.
     * Refer to the
     * <a href="Felix.html#Felix(java.util.Map)"><tt>Felix</tt></a>
     * constructor documentation for more information on framework configuration properties.
     * </li>
     ...
     * configuration properties passed into Felix.
     * </li>
     * <li><i><b>Add shutdown hook.</b></i> To make sure the framework shutdowns cleanly, the launcher installs a
     * shutdown hook; this can be disabled with the <tt>felix.shutdown.hook</tt> configuration property.
     * </li>
     ...
     * deploy at framework startup. The default is the <tt>bundle/</tt> directory of the current directory.
     * </li>
     * <li><tt>felix.auto.deploy.action</tt> - Specifies the auto-deploy actions to be found on bundle JAR files found
     * in the auto-deploy directory. The possible actions are <tt>install</tt>, <tt>update</tt>,
     * <tt>start</tt>, and <tt>uninstall</tt>. If no actions are specified, then the auto-deploy directory is not
     * processed. There is no default value for this property.
     * </li>
     * </ul>
     * </li>
     ...
     * so that they can be processed by the launcher during the framework startup process.
     * </p>
     *
     * @param args Accepts arguments to set the auto-deploy directory and/or the bundle cache directory.
    ...
    protected Properties getDefaultSystemProps() throws IOException {
        Properties properties = new Properties(super.getDefaultSystemProps());
        loadProperties(Main.class.getResource(SYSTEM_PROPERTIES_FILE_NAME), properties);
        return properties;
    }
    ...
    protected Properties getDefaultConfigProps() throws IOException {
        ...
        loadProperties(Main.class.getResource(CONFIG_PROPERTIES_FILE_NAME), properties);
        return properties;
    }
